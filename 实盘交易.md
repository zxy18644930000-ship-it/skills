---
description: 一句话跑回测/仿真/实盘/热加载 - 自动解析参数、生成配置、构建命令并执行，支持运行时动态添加品种
---
...
- 回测 / backtest → `sim --replay` 或 `sim --ctp_data`
- 仿真 / sim → `sim`
- 实盘 / live → `live`
- **热加载 / 加品种 / 添加品种 / 加入** → `hot_reload`（运行时动态添加品种到已运行的实盘进程）

**热加载关键词识别**：以下表述均匹配热加载模式：
- "加P" / "加入P" / "添加P" / "热加载P"
- "把P加进去" / "加个P" / "运行中加P"
- "热启动P" / "动态添加P"
...
**热加载模式（hot_reload）— 运行时动态添加品种**：

适用场景：实盘进程已在运行（如 `--product CF`），用户想不停机追加新品种（如 P）。

执行方式：往 `state/.add_product` 写信号文件，主循环每秒自动检测并热加载。

```bash
# 单品种热加载
cd /Users/zhangxiaoyu/Downloads/trade2026 && echo '{"product": "{PRODUCT}"}' > state/.add_product
```

多品种热加载时，**逐个写入信号文件**，每次等上一个处理完（文件被删除）再写下一个：

```bash
cd /Users/zhangxiaoyu/Downloads/trade2026 && \
  for p in {PRODUCT1} {PRODUCT2} {PRODUCT3} ...; do \
    echo "{\"product\": \"$p\"}" > state/.add_product; \
    echo "发送热加载信号: $p"; \
    while [ -f state/.add_product ]; do sleep 1; done; \
    echo "$p 已处理"; \
  done
```

品种数量**无上限**，5个、10个都行。每个品种处理约1-2秒，全程不停机。

热加载流程（由 `runner_live.py` 自动完成）：
1. 验证品种未重复、配置存在
2. 创建 ProductRuntime（executor、state_manager、strategy）
3. 恢复历史状态（如有），处理已有持仓（自动继续持有）
4. 订阅新品种合约行情
5. 推送缓存 tick 到新策略
6. 品种立即开始工作，现有品种零影响
...
📋 热加载参数:
  品种: P (棕榈油)
  信号文件: state/.add_product

🚀 执行命令:
  cd /Users/zhangxiaoyu/Downloads/trade2026 && echo '{"product": "P"}' > state/.add_product
```

**热加载示例（多品种）**：
```
📋 热加载参数:
  品种: P, SA, TA, FG, SR（5个品种）
  方式: 逐个写入信号文件，等待处理完再写下一个

🚀 执行命令:
  cd /Users/zhangxiaoyu/Downloads/trade2026 && for p in P SA TA FG SR; do ...
```

然后直接执行命令（不需要等待确认，用户已明确意图）。

**热加载后续**：执行后提示用户观察实盘日志确认热加载成功：
- 查看日志：`tail -f logs/*.log | grep "热加载\|hot_reload\|P.*策略已启动"`
- 预期日志：`[P] ProductRuntime 创建完成` → `[P] 策略已启动` → `[P] 订阅 N 个合约`
